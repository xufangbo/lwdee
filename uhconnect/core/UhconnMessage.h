...
struct MessagIdHelper {
  static std::atomic<int> seq;
};
...
struct hash<UhconnMessage> {
  size_t operator()(const UhconnMessage& msg) const {
    int ts = chrono::system_clock::to_time_t(std::chrono::system_clock::now());

    ostringstream s;
    s << msg.getSrcNodeId();
    s << msg.getDestNodeId();
    s << ts;
    s << MessagIdHelper::seq.load();

    MessagIdHelper::seq++;

    size_t id = strtoull(s.str().c_str(), NULL, 0);
    return id;
    ...
    // size_t h5 = chrono::system_clock::to_time_t(std::chrono::system_clock::now());//std::hash<std::string>()(msg.getMessageJsonStr());
