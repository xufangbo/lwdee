...
int UhconnConfig::getRouteInfo(std::map<int, NodeInfo>& map_info) {
    ...
    for (auto item : array) {
        NodeInfo node_info;
        node_info.dataPort = item.at("dport").as_int64();
        node_info.ipv4Addr = std::string(item.at("ip").as_string().c_str());
        node_info.msgPort = item.at("mport").as_int64();
        node_info.nodeId = item.at("nid").as_int64();
        node_info.dataClient = nullptr;
        node_info.msgClient = nullptr;
        node_info.msgLock = new co_mutex;
        node_info.dataLock = new co_mutex;

        map_info[node_info.nodeId] = node_info;
    }
